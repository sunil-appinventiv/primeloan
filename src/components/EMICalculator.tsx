              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="flex items-center gap-2 text-sm font-medium text-foreground">
                    <IndianRupee className="w-4 h-4 text-muted-foreground" /> Loan Amount
                  </label>
                  <span className="text-lg font-bold text-primary">{formatCurrency(loanAmount)}</span>
                </div>
                <Slider value={[loanAmount]} onValueChange={(v) => setLoanAmount(v[0])} min={500000} max={50000000} step={100000} />
                <div className="flex justify-between text-xs text-muted-foreground mt-2"><span>₹5 L</span><span>₹5 Cr</span></div>
              </div>
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="flex items-center gap-2 text-sm font-medium text-foreground">
                    <Percent className="w-4 h-4 text-muted-foreground" /> Interest Rate (p.a.)
                  </label>
                  <span className="text-lg font-bold text-primary">{interestRate.toFixed(2)}%</span>
                </div>
                <Slider value={[interestRate]} onValueChange={(v) => setInterestRate(v[0])} min={6} max={18} step={0.1} />
                <div className="flex justify-between text-xs text-muted-foreground mt-2"><span>6%</span><span>18%</span></div>
              </div>
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="flex items-center gap-2 text-sm font-medium text-foreground">
                    <Calendar className="w-4 h-4 text-muted-foreground" /> Loan Tenure
                  </label>
                ...
                <div className="flex justify-between text-xs text-muted-foreground mt-2"><span>1 Year</span><span>30 Years</span></div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-hero-gradient text-primary-foreground shadow-card-hover border-0">
            ...
            <CardContent className="space-y-6">
              <div className="text-center py-6 bg-primary-foreground/10 rounded-2xl">
                ...
                <p className="text-4xl sm:text-5xl font-bold text-gold">₹{calculations.emi.toLocaleString("en-IN")}</p>
              </div>
              ...
              <div className="p-4 bg-primary-foreground/10 rounded-xl">
                ...
                <div className="h-3 bg-primary-foreground/20 rounded-full overflow-hidden">
                  <div className="h-full bg-accent-gradient rounded-full transition-all duration-500" style={{ width: `${calculations.principalPercentage}%` }} />
                </div>
                <div className="flex justify-between text-xs mt-2">
                  <span className="flex items-center gap-1"><span className="w-2 h-2 bg-accent rounded-full" />Principal ({calculations.principalPercentage}%)</span>
                  <span className="flex items-center gap-1"><span className="w-2 h-2 bg-primary-foreground/40 rounded-full" />Interest ({calculations.interestPercentage}%)</span>
                </div>
              </div>
